...
Knight, Dragon, Minotaur, and Twinscale. The row of buttons **scrolls horizontally**:
use the mouse wheel (it scrolls the row sideways even on a plain vertical wheel or a
macOS trackpad), or drag the scrollbar, to reach the ones off the edge. See
**[Creatures](creatures.md)** for what each one does.
