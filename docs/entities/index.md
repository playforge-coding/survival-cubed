---
comments: true
---

# Bestiary

Every creature in Survival Cubed has its own page here, with a stat block and
notes on how it behaves, where it lives, and how to deal with it. For the
big-picture overview — taming, pets, combat tips — see
[Creatures](../creatures.md).

Everything in the world is an _entity_. Only your own character is driven by your
client; every other entity (animals, monsters, even other players as you see
them) is simulated by the server.

A quick reference to how fast things move: **you walk at 150 px/s**, which is
faster than every monster on foot — you can outrun almost anything in the open.
Projectiles (bones, fireballs) are the exception, flying at 130–170 px/s.

## Friendly & neutral

| Entity                         | HP     | Role                              |
| ------------------------------ | ------ | --------------------------------- |
| [The Player](player.md)        | 20     | You                               |
| [Chicken & Goat](livestock.md) | 8 / 16 | Passive livestock — drop raw meat |
| [Cat & Puppy](pets.md)         | 8 / 14 | Tameable pets                     |
| [Horse](horse.md)              | 30     | Tameable mount                    |
| [Knight](knight.md)            | 40     | Recruitable man-at-arms           |
| [Musketeer](musketeer.md)      | 34     | Recruitable ranged marksman       |
| [Mage](mage.md)                | 30     | Spellcaster made by the restore spell |
| [White Dragon](../magic.md#the-dragonian-steed-spell) | 200 | Rideable steed summoned by the dragonian steed spell |

## Hostile monsters

| Entity                                 | HP      | Where                                          |
| -------------------------------------- | ------- | ---------------------------------------------- |
| [Slime](slime.md)                      | 10      | Mountains                                      |
| [Zombie](zombie.md)                    | 40      | All biomes, night                              |
| [Dark Knight](dark-knight.md)          | 44      | All biomes, night (overworld) — drops tungsten |
| [Dark Musketeer](dark-musketeer.md)    | 40      | Summoned by the enraged demon king             |
| [Spider](spider.md)                    | 12      | Forests & caves                                |
| [Snake](snake.md)                      | 14      | Desert                                         |
| [Skeletons](skeletons.md)              | 24 / 36 | All biomes (night) / underworld ash valleys    |
| [Demons](demons.md)                    | 28 / 40 | Underworld charred expanse                     |
| [Orcs](orcs.md)                        | 50 / 30 | Underworld charred expanse                     |
| [Necromancer & Skulls](necromancer.md) | 26 / 8  | Deserts & underworld ash valleys               |
| [Ash Twister](ash-twister.md)          | 18      | Underworld ash valleys                         |

## Minibosses

| Entity                | HP  | Where                                            |
| --------------------- | --- | ------------------------------------------------ |
| [Dragon](dragon.md)   | 200 | Underworld charred expanse — very rare, **flies** |

## Bosses

| Entity                      | HP   | Where                                          |
| --------------------------- | ---- | ---------------------------------------------- |
| [Demon King](demon-king.md) | 1000 | The **arena** — one per world                  |
| [Twinscale](twinscale.md)   | 1600 | The **arena**, 5 days after the king — post-game, **flies** |
