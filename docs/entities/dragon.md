# Dragon

The underworld's rare flying **miniboss**. A long, winged serpent that wheels
through the air, chasing you down and breathing fireballs. Far tougher than
anything else in the depths — felling one is a real fight, announced by its own
**music** and **health bar**.

## Dragon

| Stat | Value |
|---|---|
| **Health** | 200 HP |
| **Size** | 31 × 17 px |
| **Speed** | 42 px/s — **flies** |
| **Damage** | 7 HP per fireball (leaves fire where it bursts) |
| **Found in** | Underworld **charred** expanse — **very rare** |
| **Activity** | All hours |

A dragon **flies**, kiting you through the air like an [enchanted
demon](demons.md#enchanted-demon) but swifter and far hardier: it closes when
you're out of range (~280 px), backs off when you crowd it (within ~150 px), and
otherwise holds station and **breathes [fireballs](#fireball)** roughly every 1.4
seconds. The fireball flies straight until it hits you or a wall and **bursts into
a lick of fire** where it lands.

Dragons are the rarest spawn in the underworld — only a handful of charred-expanse
chunks in a thousand seed one, and only where there's a **tall open cavern** for it
to fly in, so a dragon always appears aloft and in plain sight rather than buried in
rock. With 200 HP and relentless ranged fire, it's a genuine miniboss; pick your
moment, use cover, and don't get cornered while it rains fire.

### Fireball

The bolt a dragon breathes: flies at 150 px/s for up to 3 seconds, dealing 7 HP on
a direct hit and leaving a tongue of fire where it bursts.

## Music & health bar

A nearby dragon raises its own **miniboss music** and a **health bar** across the
top of the screen, both driven purely by proximity. They kick in once the dragon is
close (within ~340 px — on or near your screen) and fall away again when you put
distance between you, when the dragon is **slain**, or when you **leave the
dimension**.

## Lore

*To be written.*
